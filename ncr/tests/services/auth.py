...
class TestAttemptLogin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.get_user_patch = mock.patch(
            ...
            return_value=User(username='username', password='hashed_pass',
                              salt='salt'))
        self.get_session_by_username_patch = mock.patch(
            'ncr.services.auth.SessionService.get_session_by_username',
            return_value=None)
        ...
        self.mock_get_session_by_username = \
            self.get_session_by_username_patch.start()
    ...
    def tearDown(self):
        ...
        self.get_session_by_username_patch.stop()
        self.create_session_patch.stop()

    def test_good_login_with_session_succeeds(self):
        self.mock_get_session_by_username.return_value = Session(
            username='username',
            token='token',
        )
        ...
        assert self.mock_get_session_by_username.call_count == 1
    ...
    def test_bad_username_fails(self):
        ...
        assert self.mock_get_session_by_username.call_count == 0
        assert self.mock_gen_token.call_count == 0
        assert self.mock_create_session.call_count == 0
